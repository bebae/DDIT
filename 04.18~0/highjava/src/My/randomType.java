...
public class randomType {
    ...
    public String name(boolean three) throws IOException {
        ...
        List<String> secondName = Arrays.asList("강", "건", "경", "고", "관", "광", "구", "규", "근", "기", "길", "나", "남", "노", "다",
        ...
        if (three){
            bw.write(firstName.get(0) + secondName.get(0) + secondName.get(1));
        } else {
            bw.write(firstName.get(0));
            int nameLength = random.nextInt(9) + 1;
            for (int i = 0; i < nameLength; i++) {
                int nameArray = random.nextInt(secondName.size());
                bw.write(secondName.get(nameArray));
            }
        }
        bw.flush();
        return sw.toString();
